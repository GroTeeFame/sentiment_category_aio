...
class MongoDBHandler:
    ...
    def find_document_by_id(self, collection_name, document_id):
        ...
        try:
            ...
            logger.error(f"Error -> mongodb_handler -> find_document_by_id()")
    ...
    def find_documents_by_date(self, collection_name, start_date, end_date, query={}):
        logger.info(f"--> class MongoDBHandler, find_documents_by_date() -->")
        logger.info(f"C: {collection_name}, SD: {start_date}, ED: {end_date}, Q: {query}")
        collection = self.get_collection(collection_name)
        date_query = {
            'timestamp': {
                '$gte': start_date,
    ...
    def update_document_by_id(self, collection_name, document_id, update_values):
        ...
        try:
            ...
            logger.info(f"Error converting to ObjectId: {e}")
            logger.info(f"Error -> mongodb_handler -> update_document_by_id()")
    ...
    def delete_document_by_id(self, collection_name, document_id):
        ...
        try:
            ...
            logger.error(f"Error converting to ObjectId: {e}")
            logger.error(f"Error -> mongodb_handler -> delete_document_by_id()")
            return None
        return document
